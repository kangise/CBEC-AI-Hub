---
layout: page
title: 关于
permalink: /about/
---

# 关于 ecommerce-ai-roadmap

## 这个项目是什么

ecommerce-ai-roadmap 是 AAAI China Chapter 发起的开源项目，专注于 AI 在跨境电商领域的实际应用。

它不是链接聚合，而是提供原创的、可直接使用的实战内容：Prompt 模板、Colab Notebook、案例分析。

## 设计原则

- 三类受众精准切分：运营人（不写代码）、技术人（写代码）、管理者（定策略）
- 原创可执行内容：每个 Prompt 都经过实际业务验证，每个 Notebook 都能在 Colab 运行
- 可追踪的学习路径：每个模块都有明确的完成标志
- 免费优先：优先推荐免费和开源的工具与课程
- 社区驱动：通过 AAAI China Chapter 社群持续更新和优化

## AAAI China Chapter

AAAI China Chapter (AAAI 中国分会) 致力于推动 AI 技术在中国的应用和发展。ecommerce-ai-roadmap 是 Chapter 在跨境电商垂直领域的实践项目。

## 贡献

欢迎提交 PR 贡献 Prompt 模板、Notebook、案例或修正内容。

- [GitHub 仓库](https://github.com/kangise/ecommerce-ai-roadmap)
- [提交 Issue](https://github.com/kangise/ecommerce-ai-roadmap/issues)

## 许可证

[CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/)
