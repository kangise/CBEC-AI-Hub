...
```mermaid
flowchart TD
    START(["F4 自动化与 Agent"]) --> S1
    S1["1. 从 Prompt 到 Agent"]
    S1 --> S2
    S2["2. 自动化三层模型"]
    S2 --> S3
    S3["3. MCP 协议详解"]
...
| 内部数据 | 销售报告、运营数据 | ⚠️ 谨慎 | 企业版 API（数据不用于训练） |
...
| 资源 | 来源 | 为什么推荐 |
|------|------|-----------|
| [B4 AI Agent 与自动化](../b-developers/b4-agent-workflow.md) | CBEC-AI-Hub | 本 Hub 的技术实操模块 |
