...
> ⚠️ **关键概念**：模型参数量（如 7B = 70 亿参数）决定了所需内存。经过量化（如 Q4_K_M）后，7B 模型约占 4-5GB 内存。详见第 7 节量化技术。
...
> ⚠️ **Mac 用户注意**：当统一内存不够时，macOS 会使用 SSD swap，导致推理速度暴降 10 倍以上，而且长期大量 swap 会损耗 SSD 寿命。确保模型大小不超过可用内存的 80%。
